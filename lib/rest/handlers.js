...
const setLastModified = (res, model) => {
  res.set(`Last-Modified`, new Date(model._ts).toUTCString());
};
...
const destroy = async (req, res) => {
  ...
  await db.delete(req.params[req.type], req.token.sub, opts);
  res.status(204).end();
};
...
const update = async (req, res) => {
  ...
  const model = await db.update(req.body, req.token.sub, req.Type, opts);
  setLastModified(res, model);
  res.status(200).json(model);
};
...
const upsert = async (req, res) => {
  const opts = { ifMatch: req.headers[ifMatchHeader] };
  const model = await db.upsert(req.body, req.token.sub, req.Type, opts);
  setLastModified(res, model);
  res.status(201).json(model);
};
