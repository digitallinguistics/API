...
const createUserToken = (userRid, clientAppRid, clientAppSecret) => {
  ...
  const payload = { cid: clientAppRid, scope: 'user' };
  ...
  if (!req.query.client_id) { respond('Please include a `client_id` parameter in the querystring.', 400, res); }
    ...
    .then(app => {
      if (req.clientApp && req.clientApp.id !== app.id) {
        respond('There was a problem authentication this application.', 400, res);
      } else {
        ...
        if (req.loggedIn) { // return a DLx token
          ...
          db.login(req.user.rid).then(result => {
            if (result.status == 200) {
              ...
              req.user = new User(result.data);
              const token = createUserToken(req.user.rid, req.clientApp.rid, req.clientApp.secret);
...
// create a new DLx user token and return it
exports.oauth = (req, res) => {
  if (!(req.payload && req.payload.sub && req.payload.service && req.payload.jti)) {
    respond('Missing claims in the JWT payload. Please include sub (service ID), service (3rd party service), and jti (state).', 401, res);
  } else {
    const authReq = authRequests.filter(authReq => authReq.state === req.payload.jti)[0];
    if (!authReq) { respond('Problem matching OAuth response to original request.', 500, res); }
      ...
      .then(user => {
        user = new User(user);
        // FIXME: I think req.clientApp is null here. Current error in the last /oauth spec: Cannot read property 'secret' of null.
        const token = createUserToken(user.rid, authReq.clientApp.rid, req.clientApp.secret);
