...
// TODO: need to verify state here, but otherwise the token should already be verified by the middleware
exports.oauth = (req, res) => {
  if (!(req.payload && req.payload.sub && req.payload.service && req.payload.jti)) {
    ...
    const authReq = authRequests.filter((authReq, i, arr) => {
      if (authReq.state === req.payload.jti) {
        arr.splice(i, 1);
        return true;
      } else { return false; }
    })[0];
    if (!authReq) { respond('Problem matching OAuth response to original request.', 500, res); }
      ...
      console.log('authRequest in OAuth:', authReq);
