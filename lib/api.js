          ...
          db.login(req.user.rid).then(result => {
            if (result.status == 200) {
              ...
              const opts = {
                ...
                expiresIn: 3600,
                subject: req.user._rid
              };

              const payload = {
                cid: req.clientApp.rid,
                scope: 'user'
              };

              const token = jwt.sign(payload, req.clientApp.secret, opts);
...
// expects the JWT token in the body of the POST request, containing three claims: sub (the service ID), service (the third party service), and jwtid (the state)
// lookup auth request
// lookup user by service ID
// create a new DLx user token and return it
exports.oauth = (req, res) => {
  if (req.method !== 'POST') { respond('OAuth responses must be POST requests.', 405, res); }
};
