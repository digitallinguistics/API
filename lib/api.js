...
exports.error404 = function (req, res) {
  res.format({
    'application/json': function () {
      ...
      res.json({
        status: 404,
        details: 'Not found. The request format may be invalid. The request headers or authorization may not be properly set, or the request URL may not be formatted incorrectly. The URL you requested was: ' + req.protocol + req.host + req.originalUrl
      });
    },
...
exports.updateCollectionItems = function (req, res) {
  auth.authenticate(req, res).then(res => console.log(res)).catch(err => console.log(err));
};
