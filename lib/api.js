      ...
      if (req.loggedIn) { // return a DLx token
        ...
        db.login(req.user.rid).then(result => {
          if (result.status == 200) {
            ...
            const token = jwt.sign({ cid: req.clientApp.rid, scope: 'user' }, req.clientApp.secret, opts);
...
exports.deleteApps = (req, res) => {
  if (req.authStatus !== 'valid') { respond('invalid_request', 401, res); }
  else if (req.authScope !== 'db') { respond('Insufficient permissions to access the Apps collection.', 403, res); }
  else {
...
exports.getItem = (req, res) => {
  ...
  const successHandler = result => {
    if (result.data.permissions.public || (req.user && req.user.isViewer(result.data.id))) {
      res.format({
