...
const { addPermissions, isContributor, isPublic, isViewer } = permissions;
...
const hydrate = (data, Model = dlx.models[data.type]) => {
  ...
  return addPermissions(model).data;
...
const parseResponse = (err, res) => {
  ...
  e.status = err.substatus || err.code || 500;
  return e;
...
const createDoc = (data, options = {}) => new Promise((resolve, reject) => {
  db.createDocument(coll, data, options, (err, doc) => {
    const e = parseResponse(err, doc);
    if (e) {
      if (e.status == 409) e.message = `Document with ID "${data.id}" already exists.`;
      return reject(e);
    }
    resolve(doc);
  });
...
const deleteDoc = (...args) => new Promise((resolve, reject) => {
  ...
  db.executeStoredProcedure(sprocLink, [...args], (err, res) => {
    ...
    if (e) {
      if (e.status == 404) {
        const [id] = args;
        e.message = `No document with ID "${id}" exists.`;
      }
      return reject(e);
    }
    resolve(res);
...
const readDoc = (id, options) => new Promise((resolve, reject) => {
  ...
  db.readDocument(`${coll}/docs/${id}`, opts, (err, doc) => {
    ...
    if (e) {
      if (e.status == 404) e.message = `No document with ID "${id}" exists.`;
      return reject(e);
    }
...
const updateDoc = (...args) => new Promise((resolve, reject) => {
  ...
  db.executeStoredProcedure(sprocLink, [...args], (err, res) => {
    ...
    if (e) {
      const [data] = args;
      if (e.status == 404) e.message = `Document with ID "${data.id}" does not exist.`;
      return reject(e);
    }
    resolve(res);
  });
...
const create = async (data = {}, userId, type) => {
  ...
  model.permissions.owner.push(userId);
...
const getAll = async (userId, type, options) => {
  ...
  const { lastModified, public: pub } = options;
  ...
  const permissionsQuery = `
    ...
    AND (
      ${pub ? publicQuery : ''}
      ARRAY_CONTAINS(c.permissions.viewer, @userId)      -- User is viewer
      OR
      ARRAY_CONTAINS(c.permissions.contributor, @userId) -- User is contributor
      OR
      ARRAY_CONTAINS(c.permissions.owner, @userId)       -- User is owner
    )
  `;

  const lastModifiedQuery = `
    -- Has been modified after the value of If-Modified-Since
    AND c._ts >= @lastModified
  `;
