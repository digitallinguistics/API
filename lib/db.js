      ...
      collections.forEach(coll => {
        ...
        const checkCounter = () => new Promise((resolve, reject) => {
          ...
          const uploadCounter = res => new Promise((resolve, reject) => {
            if (res.length === 0) {
              const counterDoc = { id: 'counter', counter: 0 };
...
exports.getAppById = appId => new Promise((resolve, reject) => {
  documentdb.executeStoredProcedure(db.apps.sprocs.getItemsByIds.link, [[appId]], (err, res) => {
    if (err) { reject(err); }
    else if (!(res instanceof Array)) { reject('There was a problem retrieving information for this application ID.'); }
    else if (res.length === 0) { reject({ status: 404, details: 'An application with this ID was not found.' }); }
    else { resolve(res[0]); }
  });
});
...
exports.getUserById = (id, service) => new Promise((resolve, reject) => {
  ...
  documentdb.executeStoredProcedure(db.users.sprocs.getItemsByIds.link, [[id], getBy, { service: service }], (err, res) => {
    ...
    if (err) { reject(err); }
    else if (!(res instanceof Array)) { reject({ status: 500, details: 'There was a problem retrieving information for the requested user.' }); }
    else if (res.length === 0) { reject({ status: 404, details: `No user found with the given ID. <a class=link href=/login>Try logging in with a different service</a>, or <a class=link href=/login?register=true>create a new account</a>.` }); }
...
exports.registerUser = userInfo => new Promise((resolve, reject) => {
  ...
  documentdb.createDocument(db.users.link, user, (err, res) => {
    if (err) {
      switch (err.code) {
        case 409:
          reject({ status: 409, details: `A user with this email already exists. <a class=link href=/login>Try logging in with a different service.</a>` });
          break;
        default:
          reject(err);
      }
