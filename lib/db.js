...
exports.getById = (coll, input, opts) => new Promise((resolve, reject) => {
  ...
  const idType = (opts.idType || opts.id_type) === ('serviceId' || 'service_id') ? 'serviceId' : 'id';

  const task = () => documentdb.executeStoredProcedure(db[coll].sprocs.getItemsByIds.link, [ids, idType, { service: opts.service }], (err, res, headers) => {
    if (err) {
      if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
      else { reject(r.convert(err)); }
    } else {
