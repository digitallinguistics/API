      ...
      serverScript: function (ids) {
        ...
        var accepted = __.filter(filter, function (err, res) {
          if (err) { throw new Error(err); }
            ...
            if (res.length === 0) { response.setBody({ status: 404, error: 'Not found.', error_description: 'No item with that ID was found.' }); }
            else {
              var accepted = __.deleteDocument(res[0]._self, function (err) {
                if (err) { throw new Error(err); }
                else { response.setBody({ status: 204 }); }
              });
...
const createId = coll => new Promise((resolve, reject) => {
  ...
  const getCounter = () => new Promise((resolve, reject) => {
    const task = () => {
      documentdb.readDocument(db[coll].counterLink, (err, res, headers) => {
        if (err) {
  ...
  const increment = counterDoc => new Promise((resolve, reject) => {
    counterDoc.counter++;
    const task = () => {
      documentdb.upsertDocument(db[coll].link, counterDoc, (err, res, headers) => {
        if (err) {
          if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
          else { reject(r.convert(err)); }
        }
...
exports.createApp = appName => new Promise((resolve, reject) => {
  ...
  const task = () => {
    documentdb.createDocument(db.apps.link, app, (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else {
...
exports.delete = (rid, coll) => new Promise((resolve, reject) => {
  const link = db[coll].link + 'docs/' + rid + '/';
  const task = () => documentdb.deleteDocument(link, (err, res, headers) => {
    if (err) {
      if (+err.code === 429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
      else if (+err.code === 404) { reject(r.json('Item not found.', 404)); }
      else { reject(r.convert(err)); }
    } else { resolve(r.json('Delete operation successful.', 204)); }
  });
  task();
});
...
exports.deleteByIds = (coll, ids) => new Promise((resolve, reject) => {
  const task = () => {
    documentdb.executeStoredProcedure(db[coll].sprocs.deleteItemsByIds.link, [ids], (err, res, headers) => {
      if (err) {
        if (+err.code === 429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else if (res) {
        if (res.status < 400) { resolve(r.json('Delete operation successful.', 204)); }
        else { reject(res); }
      }
    });
  };
...
exports.get = link => new Promise ((resolve, reject) => {
  const task = () => {
    documentdb.readDocument(link, (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else { resolve(res); }
...
exports.getAppById = appId => new Promise((resolve, reject) => {
  const task = () => {
    documentdb.executeStoredProcedure(db.apps.sprocs.getItemsByIds.link, [[appId]], (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
...
exports.getById = (coll, id) => new Promise((resolve, reject) => {
  const task = () => {
    documentdb.executeStoredProcedure(db[coll].sprocs.getItemsByIds.link, [[id], 'id'], (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else { resolve(res[0]); }
...
exports.getUserById = (id, service) => new Promise((resolve, reject) => {
  ...
  const task = () => {
    documentdb.executeStoredProcedure(db.users.sprocs.getItemsByIds.link, [[id], getBy, { service: service }], (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else if (!(res instanceof Array)) { reject(r.json('There was a problem retrieving information for the requested user.')); }
      else if (res.length === 0) { reject(r.json(`No user found with the given ID. Try logging in with a different service, or create a new account.`, 404)); }
...
exports.registerUser = userInfo => new Promise((resolve, reject) => {
  ...
  const task = () => {
    documentdb.createDocument(db.users.link, user, (err, res, headers) => {
      if (err) {
        switch (err.code) {
          ...
          case 429: setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); break;
...
exports.updateUser = user => new Promise((resolve, reject) => {
  const task = () => {
    documentdb.upsertDocument(db.users.link, user, (err, res, headers) => {
      if (err) {
        if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
        else { reject(r.convert(err)); }
      } else { resolve(new User(res)); }
...
exports.upsert = (coll, input) => new Promise((resolve, reject) => {
  ...
  items.forEach((item, i, arr) => {
    ...
    const upsert = id => new Promise((resolve, reject) => {
      ...
      const task = () => {
        documentdb.upsertDocument(db[coll].link, item, (err, res, headers) => {
          if (err) {
            if (err.code === +429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
            else { reject(r.convert(err)); }
          } else {
