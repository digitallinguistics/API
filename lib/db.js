...
const makeRequest = (operation, args, handler) => {
  ...
  const processResponse = (err, res, headers, task) => {
    if (err && err.status == 429) { setTimeout(task, (+headers['x-ms-retry-after-ms']) + 1); }
...
const db = (() => {
  ...
  /**
   * A hash containing any collection-specific stored procedures to upserted on initialization.
   * @type {object}
   */
  this.collectionSprocs = {
    users: {
      checkLogin: require('./sprocs/checkLogin'),
...
exports.get = (coll, input) => new Promise ((resolve, reject) => {
  ...
  if (typeof coll !== 'string') { reject('Collection parameter must be a string.', 400); }
...
/**
 * 'Logs in' the user by updating the <code>lastActive</code> attribute.
 * @param {string}        The RID of the user to log in.
 */
exports.login = rid => new Promise((resolve, reject) => {
  const handler = (err, res) => {
    if (err) { reject(convertResponse(err)); }
    else if (res.status == 201) { resolve(jsonResponse('User activity updated.', 201)); }
  };
...
exports.logout = rid => new Promise((resolve, reject) => {
  const handler = (err, res) => {
    if (err) { reject(convertResponse(err)); }
    else if (res.status == 201) { resolve(jsonResponse('User is now logged out.', 201)); }
  };
