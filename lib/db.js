...
const destroy = async (type, id, userID, options = {}) => {
  if (type in subItems) await Promise.all(subItems[type].map(subType => deleteSubItems(subType, id, userID)));
...
const getAll = async (userID, type, options) => {
  ...
  if (typeof ifModifiedSince !== `undefined` && !Date.parse(ifModifiedSince)) {
    const e = new Error(`The If-Modified-Since header must be a valid date string.`);
    e.status = 400;
    throw e;
  }

  if (typeof maxItemCount !== `undefined` && !Number.isInteger(Number(maxItemCount))) {
...
const upsert = async (data = {}, userID, options = {}) => {
  ...
  if (data.type in parents) {
    ...
    // get parentID
    const parentIDProp = `${parents[data.type].toLowerCase()}ID`;
    const parentID     = data[parentIDProp];
    ...
    if (!isContributor(parent, userID)) {
      const e = new Error(`The user does not have permissions to add ${plurals[parents[data.type]]} to this ${parents[data.type]}.`);
      e.status = 403;
      throw e;
    }

    // make owners of parent owners of subitem
    data.permissions.owners.push(...parent.permissions.owners);
