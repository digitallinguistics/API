      ...
      if (err) { reject(convertResponse(err)); }
        ...
        collections.forEach(coll => {
          ...
          const checkCounter = () => new Promise((resolve, reject) => {
            ...
            const uploadCounter = res => new Promise((resolve, reject) => {
              if (res.length === 0) {
                ...
                const createDoc = () => documentdb.createDocument(this[coll.id].link, counterDoc, (err, res, headers) => {
                  if (err) {
                    if (err.code == 429) { setTimeout(createDoc, (+headers['x-ms-retry-after-ms']) + 1); }
          ...
          const uploadSprocs = () => new Promise((resolve, reject) => Object.keys(this.sprocs).forEach(sprocName => {
            ...
            const uploadSproc = () => documentdb.upsertStoredProcedure(this[coll.id].link, this.sprocs[sprocName], (err, res, headers) => {
              if (err) {
                if (err.code == 429) { setTimeout(uploadSproc, (+headers['x-ms-retry-after-ms']) + 1); }
                else { reject(convertResponse(err)); }
              } else {
...
exports.create = (coll, input, opts) => new Promise((resolve, reject) => {
  ...
  const makeRequests = () => items.reduce((promise, item) => {
    ...
    return promise.then(() => new Promise((resolve, reject) => {
      ...
      const create = () => documentdb.createDocument(db[coll].link, item, (err, res, headers) => {
        if (err) {
          if (err.code == 429) { setTimeout(create, (+headers['x-ms-retry-after-ms']) + 1); }
          else { reject(convertResponse(err)); }
        } else {
          results.push(res);
          resolve();
      ...
      if (opts.createId) {
        createId(coll).then(id => { item.id = id; create(); }).catch(reject);
      } else { create(); }
  ...
  /**
   * Old code for making requests in parallel (causes throttling)
   *
  items.forEach(item => {
    if (coll === 'users') { item.services = item.services || {}; }
...
exports.delete = (coll, input) => new Promise((resolve, reject) => {
  ...
  const makeRequests = () => rids.reduce((promise, rid) => {
    ...
    return promise.then(() => new Promise((resolve, reject) => {
      ...
      const destroy = () => documentdb.deleteDocument(link, (err, res, headers) => {
        if (err) {
          if (err.code == 429) { setTimeout(destroy, (+headers['x-ms-retry-after-ms']) + 1); }
  ...
  /*
   * Old code that makes requests in parallel (causes throttling)
   *
  rids.forEach(rid => {
...
exports.get = (coll, input) => new Promise ((resolve, reject) => {
  ...
  const makeRequests = () => rids.reduce((promise, rid) => {
    ...
    return promise.then(() => new Promise((resolve, reject) => {
      ...
      const get = () => documentdb.readDocument(link, (err, res, headers) => {
        if (err) {
          if (err.code == 429) { setTimeout(get, +(headers['x-ms-retry-after-ms']) + 1); }
          else if (err.code == 404) { reject(jsonResponse(`Item with RID ${rid} not found.`, 404)); }
          else { reject(convertResponse(err)); }
        } else {
          results.push(res);
...
exports.upsert = (coll, input, opts) => new Promise((resolve, reject) => {
  ...
  /**
   * Old code which makes requests in parallel (causes throttling)
   *
  items.forEach(item => {
