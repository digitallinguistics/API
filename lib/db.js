        ...
        collections.forEach(coll => {
          ...
          const checkCounter = () => new Promise((resolve, reject) => {
            ...
            const readCounter = () => new Promise((resolve, reject) => {
              documentdb.executeStoredProcedure(this.collections[coll.id].sprocs.getItemsByIds.link, [['counter'], 'id'], (err, res) => {
            ...
            const uploadCounter = res => new Promise((resolve, reject) => {
              if (res.length === 0) {
                ...
                const createDoc = () => documentdb.createDocument(this.collections[coll.id].link, counterDoc, (err, res, headers) => {
                  if (err) {
                    if (err.code == 429) { setTimeout(createDoc, (+headers['x-ms-retry-after-ms']) + 1); }
                    else { reject(convertResponse(err)); }
                  } else { this.collections[coll.id].counterLink = res._self; resolve(res); }
                });
                createDoc();
              } else { this.collections[coll.id].counterLink = res[0]._self; resolve(); }
            });
          ...
          const uploadSprocs = () => new Promise((resolve, reject) => Object.keys(this.sprocs).forEach(sprocName => {
            ...
            const uploadSproc = () => documentdb.upsertStoredProcedure(this.collections[coll.id].link, this.sprocs[sprocName], (err, res, headers) => {
              if (err) {
