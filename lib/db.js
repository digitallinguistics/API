...
const db = (() => {
   ...
   * @type {string}
   */
  this._rid = global.env === 'production' ? credentials.documentdb.rid : credentials.documentdb.devRid;

  /**
   * The self link for the database
   * @type {string}
   */
  this.link = (() => 'dbs/' + this._rid + '/')();
        ...
        const upsertCollectionSprocs = () => new Promise((resolve, reject) => {
          if (coll.id in this.collectionSprocs) {
            ...
            Object.keys(this.collectionSprocs[coll.id]).forEach(sprocName => {
              const handler = (err, res) => {
                if (err) { reject(convertResponse(err)); }
                this.collections[coll.id].sprocs[sprocName] = { link: res._self, _rid: res._rid };
                sprocsUploaded++;
 ...
 * @param   {object} [opts]                          An optional options hash.
 * @prop    {string} [opts.idType|opts.id_type]      <code>id</code> (including email), or <code>serviceId</code> (also accepts <code>service_id</code>). Defaults to <code>id</code>.
...
exports.getById = (coll, input, opts) => new Promise((resolve, reject) => {
  ...
  opts.idType = opts.idType === 'email' ? 'id' : opts.idType;
...
exports.upsert = (coll, input, opts) => new Promise((resolve, reject) => {
  ...
  if (coll === 'users') { items.forEach(item => item.services = item.services || {}); }
  const results = [];
