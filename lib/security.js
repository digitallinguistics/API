...
exports.requireOAuthToken = (req, res, next) => {
  if (req.tokenError) { respond(req.tokenError, 401, res); }
    ...
    try {
      ...
      if (!attrs.every(attr => attr in payload)) { respond('Token must include the aud, exp, iat, and jti claims in the payload.', 401, res); }
      else {
...
exports.verifyUserToken = (req, res, next) => {
  ...
  else if (!req.headers.authorization) { req.tokenError = jsonResponse('Access token must be present in the Authorization header.', 401); next(); }
  else {
    try {
      const opts = {
        algorithms: ['HS256'],
