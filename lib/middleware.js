...
exports.manageLogins = (req, res, next) => {
  ...
  req.loggedIn = (() => {
    if (req.signedCookies.user) {
      ...
      db.getUser(credentials.decrypt(req.headers.authorization.replace(/(B|b)earer /, '')))
  ...
  if (req.query.logout) {
    ...
    db.getUser(req.signedCookies.user || credentials.decrypt(req.headers.authorization.replace(/(B|b)earer /, '')))
