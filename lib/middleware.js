  ...
  if (req.token) {
    try {
      ...
      const getClientApp = rid => new Promise((resolve, reject) => {
        db.get('apps', rid).then(app => {
          req.claims.clientApp = new ClientApp(app);
        }).then(resolve).catch(reject);
      });

      /** Attempts to retrieve the claimed User */
      const getUser = rid => new Promise((resolve, reject) => {
        db.get('users', rid).then(user => req.claims.user = new User(user)).then(resolve).catch(reject);
      });

      if (req.claims.cid) { tasks.push(getClientApp(req.claims.cid)); }
      if (req.claims.sub) { tasks.push(getUser(req.claims.sub)); }

      Promise.all(tasks).then(() => next()).catch(err => {
        req.tokenError = convertResponse(err); next();
      });

    } catch (err) {
      if (err.name === 'TokenExpiredError') { req.authStatus = 'expired'; req.tokenError = convertResponse('The access token is expired.', 419); }
      else if (err.name === 'JsonWebTokenError') { req.tokenError = convertResponse(err.message, 401); }
      else { req.tokenError = convertResponse(err, null); }
      next();
    }
  } else {
    next();
