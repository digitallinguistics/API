...
exports.parser = (req, res, next) => {
  ...
  const getUser = (id, idType) => {
    ...
    if (idType === 'rid') {
      ...
      db.getById('users', id, { idType: 'id' }).then(processUser).catch(err => respond(err, null, res));
    } else {
        ...
        const checkToken = () => {
          jwt.verify(token, key, (claims.scope === 'db' ? adminOpts : userOpts), (err, payload) => {
            if (err && err.name === 'TokenExpiredError') { req.authStatus = 'expired'; next(); }
              ...
              if (payload.scope === 'db' && !payload.sub) { respond('Sub claim missing.', 401, res); }
              else if (payload.scope === 'db' && !payload.service) { respond('Service claim missing.', 401, res); }
              else if (payload.scope === 'db' && payload.sub !== 'dlx-org' && !payload.jti) { respond('JWT ID (state) claim missing.', 401, res); }
              else if (payload.scope === 'db' && payload.sub === 'dlx-org') { next(); }
              else if (payload.scope === 'db' && payload.service) { getUser(payload.sub, 'id'); }
