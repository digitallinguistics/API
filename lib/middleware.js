...
exports.parser = (req, res, next) => {
  ...
  .then(user => {
    ...
    req.user = user;
    if (user.lastActive > (Date.now() - 14400000)) { req.loggedIn = true; }
    next();
  }).catch(err => respond(err, null, res));

  const verifyToken = token => {
    jwt.verify(token, credentials.secret, (err, payload) => {
      if (err && err.name === 'TokenExpiredError') { req.authStatus = 'expired'; }
      else if (err && err.name === 'JsonWebTokenError') { next(); }
