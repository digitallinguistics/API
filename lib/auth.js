...
exports.oauth = (req, res) => {
  ...
  if (req.cookies.register) { authReq.register = true; }
    ...
    return new Promise((resolve, reject) => {
      ...
      var tokenReq = https.request(options, res => {
        ...
        res.on('end', () => {
          ...
          resolve({ serviceId: authReq.serviceId, service: authReq.service });
        });
      });
  ...
  const redirect = user => {
    ...
    if (authReq.register) { res.redirect('/register' + stateParam); }
    else { res.redirect(authReq.redirect + stateParam); }
  };
