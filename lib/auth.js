...
exports.grant = (req, res) => {
  ...
  if (!req.query.client_id) {
    renderError({ status: 400, details: `Please provide a <code>client_id</code> parameter in the query string. The client ID for your application can be found on the <a class=link href=/developer>Developer page</a> under <em>Application ID</em>. More details on authentication are available <a class=link href=${config.package.homepage}>here</a>.` }, res);
  } else if (!req.query.redirect_uri) {
    renderError({ status: 400, details: `Please provide a <code>redirect_uri</code> parameter in the query string. More details on authentication are available <a class=link href=${config.package.homepage}>here</a>.` }, res);
  } else if (!req.query.response_type || req.query.response_type !== 'token') {
    renderError({ status: 400, details: `Please provide a <code>response_type</code> parameter in the query string, with a value of <code>token</code>. More details on authentication are available <a class=link href=${config.package.homepage}>here</a>.` }, res);
  } else {
    ...
    const authenticate = app => {
      return new Promise((resolve, reject) => {
        if (app) {
          req.url = URL.parse(req.url, true).query.redirect_uri;
          exports.authenticate(req, res)
          .then(resolve)
          .catch(reject);
        } else {
          reject({ status: 400, details: 'No app with that ID found.' });
        }
      });
    };
...
exports.oauth = (req, res) => {
  ...
  if (!authReq) {
    renderError('There was a problem authenticating the user. Please try your request again.', res); }
  else {
