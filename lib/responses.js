...
(function () {
   ...
   */
  this.convertResponse = (data, status) => {
    status = status || data.status || data.code || data.statusCode || data.errorCode || 500;
    data = data ? data.data || data.error_description || data.details || data.message || data : '';
  ...
  this.jsonResponse = (data, status) => {
    ...
    if (res.status < 400) { res.data = data; }
      ...
      switch (res.status) {
        ...
        case 409: res.error = 'Data conflict.'; break;
      ...
      if (res.error_description === 'invalid_request') { res.error_description = res.error; res.error = 'invalid_request'; }
    }
    return res;
  };
   ...
   */
  this.respond = (data, status, res) => {
    const response = this.convertResponse(data, status);
