...
const getLanguages = async (req, res) => {
  ...
  const opts = {
    continuation: req.headers[`x-dlx-continuation`],
    maxItemCount: req.headers[`x-dlx-max-item-count`],
  };

  const languages = await db.getAll(req.query.ids, req.token.sub, `Language`, opts);

  if (languages.continuation) res.set(`x-dlx-continuation`, languages.continuation);
  Reflect.deleteProperty(languages, `continuation`);
