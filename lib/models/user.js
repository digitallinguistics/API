...
class User {
  ...
  checkToken (token) {
    ...
    else if (token === this.dlxToken || decrypt(token) === this.rid) { return 'valid'; }
    else { return 'invalid'; }
  }
  ...
  updateToken () {
    console.log('dlxToken:', this.dlxToken);
    console.log('rid:', this.rid);
    console.log('encrypted rid:', encrypt(this.rid));
    this.dlxToken = encrypt(this.rid);
    const d = new Date();
