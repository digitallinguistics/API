...
class User {
  ...
  checkToken (token) {
    if (!token) { return 'missing'; }
      ...
      try {
        const payload = jwt.verify(token, credentials.secret);
        if (payload) { return 'valid'; }
      } catch (err) {
        if (err.name === 'TokenExpiredError') { return 'expired'; }
        else if (err.name === 'JsonWebTokenError') { return 'invalid'; }
      }
    }
  }
  ...
  updateToken () {
    const payload = { rid: this.rid, exp: Date.now() + 3600000 };
    this.dlxToken = jwt.sign(payload, credentials.secret);
    this.lastActive = Date.now();
  }
