...
function destroy(id, userId, { ifMatch } = {}) {
  ...
  const accepted = __.readDocument(`${link}/docs/${id}`, (err, doc, info) => {
    ...
    if (info.notModified) {
      throw {
        code:    412,
        message: `Outdated ETag. Please try the delete request again with the latest version of the resource.`,
      };
    }
    ...
    if (doc.permissions.owner.includes(userId)) {
      ...
      const accepted = __.upsertDocument(link, doc, opts, err => {
