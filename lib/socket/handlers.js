...
module.exports = socket => {
  ...
  const add = async (type, data = {}, options = {}, callback = defaultCallback) => {
    ...
    if (type in parents) {
      ...
      const key      = `${parents[type].toLowerCase()}ID`;
      const parentID = opts[key] || data[key];

      if (!parentID) {
        const e  = new Error(`Please provide a ${key}.`);
        e.status = 400;
        throw e;
      }

      const parent      = await db.get(parentID, token.sub);
      data[`${type}ID`] = parent.id;
  ...
  const update = async (data, options = {}, callback = defaultCallback) => {
    ...
    if (data.type in parents) Reflect.deleteProperty(data, `${parents[data.type]}ID`);
  ...
  const upsert = async (data, options = {}, callback = defaultCallback) => {
    ...
    if (data.type in parents) {
      ...
      const key      = `${parents[data.type].toLowerCase()}ID`;
      const parentID = opts[key] || data[key];
  ...
  const deleteLanguage = async (languageID, options = {}, callback = defaultCallback) => {
    const { cb, opts } = parseArgs(options, callback);
    const token        = socket.decoded_token;
    const res          = await db.deleteLanguage(languageID, token.sub, opts);
    cb(undefined, res, { status: 204 });
    socket.broadcast.emit(`delete`, languageID, res.type);
  };
  ...
  const getLexemes = async (languageID, options = {}, callback = defaultCallback) => {
    ...
    const lexemes      = await db.getAll(socket.decoded_token.sub, `Lexeme`, opts);
