...
module.exports = socket => {
  ...
  const add = async (...args) => {
    ...
    const opts  = args.find(arg => arg instanceof Object && arg !== type && arg !== data && arg !== cb) || {};
  ...
  const getAll = async (type, options = {}, callback = defaultCallback) => {
    ...
    if (type in parents) {
      ...
      if (opts[parentIDProp]) opts.parentID = opts[parentIDProp];
    }
  ...
  const wrap = handler => (...args) => handler(...args).catch(err => {
    if (config.logErrors) console.error(e);
