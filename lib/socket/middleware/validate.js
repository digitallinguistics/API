...
module.exports = (socket, handlers) => socket.use((packet, next) => {
  ...
  if (event.includes(`update`)) {
    ...
    if (!(typeof data.id === `string` && data.type)) {
      const err  = new Error(`The "id" and "type" properties are required for partial update operations.`);
      err.status = 400;
      return cb(err);
    }
  }
  ...
  if (event.includes(`upsert`)) {
    const [, data] = packet;
    if (!(typeof data.id === `string` || data.type)) {
      const err  = new Error(`Either the "id" or "type" property must be provided for upsert operations.`);
      err.status = 400;
      return cb(err);
    }
  }
