...
const queryDocs = (querySpec, continuation) => new Promise((resolve, reject) => {
...
const getAll = async ({ continuation, ids = [] }, userId, type) => {
  ...
  const docs  = await queryDocs(querySpec, continuation);
  const Model = dlx.models[type];
